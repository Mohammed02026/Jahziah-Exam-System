...
@php
    ...
    foreach (['exam_students', 'exam_user', 'exam_student'] as $tableName) {
        if (\Illuminate\Support\Facades\Schema::hasTable($tableName)) {
            $selectedStudentIds = \Illuminate\Support\Facades\DB::table($tableName)
    ...
    $assignTo = old('assign_to', count($selectedStudentIds) > 0 ? 'selected' : 'all');
    $oldStudentIds = array_map('intval', old('student_ids', $selectedStudentIds));
@endphp

<div class="container py-4">

    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="exam-page-title m-0">
            {{ __('app.edit_exam') }} #{{ $exam->id }}
        </h2>

        <a class="btn btn-outline-secondary" href="{{ route('instructor.exams.show', $exam) }}">
            {{ __('app.back') }}
        </a>
    </div>
    ...
    <div class="card shadow-sm exam-card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('instructor.exams.update', $exam) }}">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="course_id" id="courseSelect" class="form-select" required>
                            @foreach($courses as $c)
                                <option value="{{ $c->id }}" @selected(old('course_id', $exam->course_id) == $c->id)>
                                    {{ $c->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        <label class="form-label">
                            {{ __('app.topic') === 'app.topic' ? 'Topic' : __('app.topic') }}
                        </label>
                        ...
                        <select name="topic_id" id="topicSelect" class="form-select" required>
                            ...
                            @foreach($topics as $topic)
                                <option value="{{ $topic->id }}"
                                        ...
                                        @selected(old('topic_id', $exam->topic_id) == $topic->id)>
                                    {{ $topic->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="form-text">
                            اختر الموضوع أو المحور المرتبط بهذا الاختبار.
                        </div>
                    </div>
                ...
                <div class="row g-3 mt-1">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select" required>
                            <option value="draft" @selected(old('status', $exam->status) === 'draft')>
                        ...
                        @forelse($students as $student)
                            <div class="form-check mb-2">
                                <input class="form-check-input student-checkbox"
                                       ...
                                       @checked(in_array((int) $student->id, $oldStudentIds))>
